...
const WithdrawalHistoryScreen = (props) => {
    const { authData } = useAuth();

    const { data, error, loading, refetch } = useQuery(GET_WITHDRAWAL_HISTORY, {
        variables: {
    ...
    const onLoadMoreHandler = () => {
        refetch({
            userId: authData?.user?._id || null,
            queryString: {
                limit: String(data.getWithdrawalHistory.length + 2),
            },
        });
    };
    ...
    if (data?.getWithdrawalHistory?.length <= 0) {
        return (
            <Box
                w={'100%'}
                h={'100%'}
    ...
    return (
        <Wrapper>
            ...
            {data?.getWithdrawalHistory?.length > 0
                ? data.getWithdrawalHistory.map((item) => (
                      <WithdrawalHistoryItem withdrawal={item} key={item._id} />
                  ))
                : null}
